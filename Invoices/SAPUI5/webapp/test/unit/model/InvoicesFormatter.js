...
sap.ui.define([
    ...
    function (InvoicesFormatter, ResourceModel) {
        ...
        QUnit.test("Should return the Invoice Status", function(assert){
                let oModel = this.stub();  // permite simular controlador y vista
                ...
                //Assert
            assert.strictEqual(fnIsolatedFormatter("A"), "New", "The invoice status for A is OK");
            assert.strictEqual(fnIsolatedFormatter("B"), "In Progress", "The invoice status for B is OK");
            assert.strictEqual(fnIsolatedFormatter("C"), "Done", "The invoice status for C is OK");
